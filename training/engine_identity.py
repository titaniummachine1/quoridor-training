...
STAMP = ROOT / "training" / "data" / "engine_stamp.json"
...
def assert_binary_identity(*, write_if_missing: bool = False) -> dict:
    ...
    if expected is None:
        if write_if_missing:
            return current
        raise RuntimeError(
            f"engine stamp missing: {STAMP}. Run training/engine_identity.py --write "
...
def assert_parity_6_of_6() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "training" / "parity_check.py")],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
        timeout=120,
