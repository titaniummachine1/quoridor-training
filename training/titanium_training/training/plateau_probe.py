...
def evaluate_promotion_gate(*, force: bool = False) -> tuple[bool, str, bool]:
    ...
    try:
        ...
        from titanium_training.training.guards import rebuild_titanium_release, net_weights_size_ok, HALFPW_WEIGHT_BYTES
        ...
        if DEPLOYED_WEIGHTS.exists():
            DEPLOYED_WEIGHTS.write_bytes(backup)
        try:
            from titanium_training.training.guards import rebuild_titanium_release
            rebuild_titanium_release()
        except Exception:
            pass
...
def nnue_status_compact() -> str:
    ...
    try:
        from titanium_training.training.learning_metrics import collect_learning_report
