...
def current_v15_rating(manifest: dict | None = None) -> int | None:
    from tools.maintenance.manifest import CURRENT_ENGINE, entity_label, load_manifest
...
def self_matchup_win_rate(manifest: dict | None = None) -> tuple[float, int] | None:
    ...
    from tools.maintenance.manifest import load_manifest, matchup_key
...
def count_db_games(db_path: Path | None = None) -> int:
    import sqlite3
    from tools.datagen.datagen import DB_PATH
...
def maybe_deploy_after_train(*, force: bool = False) -> tuple[bool, str]:
    ...
    from titanium_training.training.plateau_probe import evaluate_promotion_gate
