...
def main() -> int:
    ...
    run_dir = Path(args.out_dir) if args.out_dir else TRAINING_ROOT / "runs" / f"smoke_{stamp}"
    ...
    artifact = verify_candidate_artifacts(ACTIVE_TEACHER_DATASET, root=REPO_ROOT, sample_policy_records=min(100, args.max_samples))
    if not artifact.passed:
        return _fail(f"artifact verification: {artifact.to_dict()}")
    loader = run_loader_smoke_audit(ACTIVE_TEACHER_DATASET, root=REPO_ROOT)
    ...
    from tools.datagen.datagen import expand_games
    from titanium_training.store.lib import connect_db, moves_from_u8_blob
    ...
    rc = subprocess.call(train_cmd, cwd=str(REPO_ROOT))
    if rc != 0:
    ...
    rc = subprocess.call(resume_cmd, cwd=str(REPO_ROOT))
    if rc != 0:
    ...
    rc = subprocess.call(
        [sys.executable, str(TRAINING_ROOT / "nnue_cli.py"), "export", "--checkpoint", str(ckpts[-1]), "--output", str(export_out)],
        cwd=str(REPO_ROOT),
        env={**dict(__import__("os").environ), "PYTHONPATH": str(TRAINING_ROOT)},
    )
    ...
    meta["checkpoint"] = str(ckpts[-1].relative_to(REPO_ROOT))
    (run_dir / "smoke_meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    print(f"\nSMOKE COMPLETE — run dir: {run_dir.relative_to(REPO_ROOT)}")
    return 0
