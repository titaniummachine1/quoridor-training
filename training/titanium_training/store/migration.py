...
from titanium_training.store.config import (
    ARCHIVE,
    CANONICAL_DB,
    DATA_DIR,
    EXPORT_DIR,
...
from titanium_training.store.guards import LegacyTrainingSourceError
from titanium_training.store.lib import (
    ImportStats,
    db_summary,
...
from titanium_training.store.state import MOVE_SCHEMA_VERSION, POSITION_SCHEMA_VERSION
        ...
        for path in base.rglob("*"):
            ...
            try:
                if path.suffix.lower() == ".jsonl":
                    from titanium_training.store.lib import classify_jsonl_object, jsonl_first_object
                    ...
                    from titanium_training.store.lib import parse_games_text
...
def run_full_audit(db_path: Path = GAME_STORE_DB) -> dict[str, Any]:
    from titanium_training.store.lib import audit_database
...
def prove_rebuild(migration_run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        from titanium_training.store.config import CANONICAL_DIR
...
def export_training_smoke(db_path: Path = GAME_STORE_DB) -> dict[str, Any]:
    from titanium_training.store.lib import export_training_rows
...
def shard_ingestion_smoke(db_path: Path = GAME_STORE_DB) -> dict[str, Any]:
    from titanium_training.store.lib import BinaryShardWriter, import_binary_shard
