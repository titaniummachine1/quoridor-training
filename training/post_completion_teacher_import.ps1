...
python training/position_store.py --teacher-db $TeacherDb audit-teacher-store `
...
$before = python -c "from pathlib import Path; from position_store_lib import db_summary; import json; print(json.dumps(db_summary(Path('$TeacherDb'))))"
python training/position_store.py --teacher-db $TeacherDb import-friend-rust --threads 8 2>&1 `
| Tee-Object "$reportDir/teacher_idempotence_rerun_$stamp.log"
$after = python -c "from pathlib import Path; from position_store_lib import db_summary; import json; print(json.dumps(db_summary(Path('$TeacherDb'))))"
...
python training/position_store.py teacher-semantic-checksum --teacher-db $TeacherDb `
